# game_controller_memory.py
# 游戏控制器模块 - 带上下文记忆版本（仅使用阿里云 API）
...
class GameControllerMemory:
    """游戏控制器类 - 带上下文记忆（JSON 格式交互）"""
    
    def __init__(self):
        self.game = ShibaGame()
        # 强制使用阿里云后端
        self.ai_service = AIService(backend="aliyun")
        # 对话历史记录
...
请按照规定的格式输出。"""
        ...
        # 使用带记忆的调用
        response = self.ai_service.call_with_memory(
            user_prompt, 
            system_prompt, 
            self.conversation_history
        )
...
请按照规定的格式输出。"""
    ...
    def stream_player_action(self, action: str):
        ...
        # 先获取完整回复（带历史记录）
        full_response = self.ai_service.call_with_memory(
            user_prompt,
            system_prompt,
            self.conversation_history
        )
    ...
    def get_fallback_response(self):
        """获取备用回复（当 AI 调用失败时）"""
        return f"""
【状态栏：深度 -{self.game.depth}m 能量 {self.game.energy}% 负重 {self.game.current_weight}/{self.game.max_weight} 金币 {self.game.gold} | 知识馆 L{self.game.hall_level}】
...
氧气耗尽！阿柴的眼前渐渐模糊，潜水服上的求救信号自动发射。搜救队的狼犬们迅速赶到，将昏迷的阿柴拉回了海面。

**损失**：本次捕获的所有食材都在慌乱中丢失了（{len(self.game.inventory)}条鱼）。

阿柴在居酒屋醒来，头上的绿方巾歪了。它垂头丧气地摇了摇尾巴，但很快又振作起来——海面还是那么蓝，新的冒险还在等待！
...
背包已清空。"""
        ...
        # 升级逻辑
        for equip_name in ["氧气瓶", "潜水服", "伸缩爪", "推进器", "探测器", "居酒屋"]:
